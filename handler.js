            ...
            case 'remove':
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                       let pp = 'https://telegra.ph/file/4ebea141a2d110c9959f6.jpg'
                        try {
                            pp = await this.profilePictureUrl(user, 'image')
                        } catch (e) {
                        } finally {
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'thank you for joining the group @user').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                (chat.sBye || this.bye || conn.bye || 'Goodbye @user!')).replace('@user', '@' + user.split('@')[0])
                                this.sendButtonImg(id, pp, text, "ᯤ ʟᴜᴍɪɴᴏᴜs-ᴍᴅ", "ᴀᴡᴏᴋᴀᴡᴏᴋ🐦", "bebanjoin", null)
                                }
                    }
                }
                break
...
global.dfail = (type, m, conn) => {
  ...
  let msg = {
    rowner: `Perintah ini hanya untuk developer bot`,
    owner: `Perintah ini hanya untuk owner bot`,
    mods: `Perintah ini hanya untuk moderator bot`,
    premium: `Fitur ini hanya dapat digunakan user *Permium*`,
    group: `Fitur ini hanya dapat digunakan didalam grup!!`,
    private: `Fitur ini hanya dapat digunakan diprivate chat`,
    admin: `Fitur ini hanya tersedia untuk admin grup!!`,
    botAdmin: `Jadi kan bot admin terlebih dahulu`,
